...
class Example:
    def __init__(self, data, max_length=-1):
        ...
        self.data['char_indices'] = eval(str(self.data['char_indices']))  
...
class DataCollatorForASTE:
    ...
    def tokenizer_function(self, examples):
        ...
        char_indices = [example['char_indices'] for example in examples]
        max_sent_len = max(len(sent) for sent in char_indices)  
        max_char_len = max(len(word) for sent in char_indices for word in sent)
        pad_word = []  
        padded_chars = []

        for sent in char_indices:
            if len(sent) < max_sent_len:
                padded_sent = sent + [[0] * max_char_len] * (max_sent_len - len(sent))
            else:
                padded_sent = sent[:max_sent_len]
            
            padded_sent_words = []
            for word in padded_sent:
                if len(word) < max_char_len:
                    padded_word = word + [0] * (max_char_len - len(word))
                else:
                    padded_word = word[:max_char_len]
                padded_sent_words.append(padded_word)
            
            padded_chars.append(padded_sent_words)
        ...
        batch_encodings['char_indices'] = torch.tensor(padded_chars, dtype=torch.long)
