...
class ASTE(pl.LightningModule):
    def __init__(self, hparams, data_module):  
        ...
        self.config.lstm_dim = self.hparams.lstm_dim
        self.config.pos_dim = self.hparams.pos_dim
        ...
        char_to_idx = json.load(open(os.path.join(self.hparams.data_dir, 'char2idx.json')))
        self.char_embedding = nn.Embedding(len(char_to_idx), 16)
        self.char_len = len(char_to_idx)

        if self.config.use_interaction:
            self.model = BDTFModel.from_pretrained(self.hparams.model_name_or_path, config=self.config, gen_embed=self.general_embedding, domain_embed=self.domain_embedding, char_vocab_size=self.char_len)
    ...
    def load_model(self):
        ...
        self.model = BDTFModel.from_pretrained(dir_name, config=self.config, gen_embed=self.general_embedding, domain_embed=self.domain_embedding,char_vocab_size=self.char_len)
    ...
    def training_step(self, batch, batch_idx):
        outputs = self(**batch)
        loss = outputs['table_loss_S'] + outputs['table_loss_E'] + outputs['pair_loss'] + outputs['cl_loss']
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--interaction_num", type=int, default=3)
        parser.add_argument("--use_interaction", type=bool, default=True)
...
class MemoryMonitor(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        if torch.cuda.is_available():
            ...
            pl_module.log('gpu/max_memory', max_memory)
            pl_module.log('gpu/current_memory', current_memory)
            
            print(f" Epoch {trainer.current_epoch} - GPU Memory: Max={max_memory:.2f}GB, Current={current_memory:.2f}GB")
    
    def on_train_end(self, trainer, pl_module):
        print("\n" + "="*50)
        print("Training Finished! GPU Memory Usage Summary:")
        for stats in self.memory_history:
            print(f"Epoch {stats['epoch']}: {stats['max_memory_gb']:.2f} GB")
        print("="*50)
...
def main():
    ...
    kwargs = {
        ...
        'callbacks': [memory_monitor, logging_callback],
        'logger': True,
    ...
    analyze_memory_usage(memory_monitor.memory_history)

    def analyze_memory_usage(memory_history):
        if not memory_history:
            return
            
        max_memories = [stats['max_memory_gb'] for stats in memory_history]
        avg_memory = sum(max_memories) / len(max_memories)
        max_memory = max(max_memories)
        
        print(f"\nMemory usage analysis:")
        print(f"Average peak memory: {avg_memory:.2f} GB")
        print(f"Max peak memory: {max_memory:.2f} GB")
        print(f"Fluctuation range: {min(max_memories):.2f} - {max_memory:.2f} GB")
        
        
        if max_memories[-1] > max_memories[0] * 1.2:  
